...
class SklearnPipelineFeatureGroup(AbstractFeatureGroup):
    ...
    def calculate_feature(cls, data: Any, features: FeatureSet) -> Any:
        ...
        for feature_name in features.get_all_names():
            ...
            # Try to load existing fitted pipeline from artifact using proper mloda lifecycle
            fitted_pipeline = None
            if features.artifact_to_load:
                artifact = cls.load_artifact(features)
                if artifact and "fitted_transformer" in artifact:
                    fitted_pipeline = artifact["fitted_transformer"]
                    # Verify that the pipeline matches the current configuration
                    if not cls._pipeline_matches_config(fitted_pipeline, pipeline_config):
                        fitted_pipeline = None  # Force retraining
            ...
            if fitted_pipeline is None:
                ...
                # Save the fitted pipeline as artifact using proper mloda lifecycle
                if features.artifact_to_save:
                    ...
                    features.save_artifact = artifact_data
