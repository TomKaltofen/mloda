...
class TestPandasSklearnPipelineFeatureGroup:
    ...
    def test_calculate_feature_end_to_end(self, mock_saver: Any, mock_loader: Any) -> None:
        ...
        # Verify the artifact data contains expected keys
        assert "fitted_transformer" in features.save_artifact
        assert "feature_names" in features.save_artifact
        assert "pipeline_name" in features.save_artifact
        assert "training_timestamp" in features.save_artifact
